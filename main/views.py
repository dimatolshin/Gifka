...
class AddTextToGif(APIView):
    def post(self, request):
        ...
        image_path = '/home/dima_tolshin/PycharmProjects/Gif/mysite/media/1.jpg'
        original_image = Image.open(image_path)

        # Путь к шрифту
        font_path = '/home/dima_tolshin/PycharmProjects/Gif/mysite/font/mons.ttf'
        try:
            font_size = 45
            font = ImageFont.truetype(font_path, font_size, encoding='unic')
        except IOError:
        ...
        frames = []
        num_frames = 30  # Количество кадров для анимации

        # Создаем кадры анимации
        for i in range(num_frames):
            # Создаем копию исходного изображения для каждого кадра
            frame = original_image.copy()
            draw = ImageDraw.Draw(frame)

            # Вычисляем смещение текста
            offset = int((text_width + (right - left)) * i / num_frames) - text_width
            text_x = left + offset
            text_y = top + (bottom - top - text_height) // 2
        ...
        # Сохраняем GIF в память
        gif_io = io.BytesIO()
        frames[0].save(gif_io, format='GIF', save_all=True, append_images=frames[1:], loop=0, duration=100)
        gif_io.seek(0)

        # Отправляем файл как ответ
        response = HttpResponse(gif_io, content_type='image/gif')
        response['Content-Disposition'] = 'attachment; filename="animated_text.gif"'
        return response
